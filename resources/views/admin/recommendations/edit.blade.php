    ...
    <div class="py-6 sm:py-10">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="overflow-hidden rounded-[2rem] border border-white/70 bg-white/75 p-6 shadow-xl shadow-slate-200/50 backdrop-blur-xl dark:border-white/10 dark:bg-white/5 dark:shadow-black/20 sm:p-8">
                <form method="POST" action="{{ route('admin.recommendations.update', $recommendation->id) }}" class="space-y-6">
                    @csrf
                    @method('PUT')

                    <div>
                        <label class="block text-sm font-bold text-slate-700 dark:text-slate-300">Kategori Depresi</label>
                        <x-custom-select name="depression_id" placeholder="Pilih Kategori" :selected="old('depression_id', $recommendation->depression_id)" required>
                    ...
                    <div>
                        ...
                        <input name="title" value="{{ old('title', $recommendation->title) }}" placeholder="Contoh: Konsultasi Psikolog / Tips Relaksasi" class="mt-2 block w-full rounded-xl border-slate-200 bg-white/70 text-sm shadow-sm focus:border-teal-500 focus:ring-teal-500 dark:border-white/10 dark:bg-slate-900/50" required />
                        @error('title')
                            <p class="mt-1 text-xs text-rose-600 dark:text-rose-400 font-semibold">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="content" rows="6" placeholder="Jelaskan langkah-langkah yang direkomendasikan secara detail..." class="mt-2 block w-full rounded-xl border-slate-200 bg-white/70 text-sm shadow-sm focus:border-teal-500 focus:ring-teal-500 dark:border-white/10 dark:bg-slate-900/50" required>{{ old('content', $recommendation->content) }}</textarea>
                        @error('content')
                            <p class="mt-1 text-xs text-rose-600 dark:text-rose-400 font-semibold">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-center gap-3">
                        <input id="is_active" name="is_active" value="1" type="checkbox" @checked(old('is_active', $recommendation->is_active)) class="h-4.5 w-4.5 rounded border-slate-300 text-teal-600 focus:ring-teal-500 dark:border-white/10 dark:bg-slate-900" />
