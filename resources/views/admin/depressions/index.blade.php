<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-100 leading-tight">Kategori Depresi</h2>
            <a href="{{ route('admin.depressions.create') }}" class="inline-flex items-center rounded-lg bg-sky-600 px-4 py-2 text-sm font-semibold text-white hover:bg-sky-700">Tambah</a>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
                <form method="GET" class="flex flex-col gap-3 sm:flex-row sm:items-center">
                    <input name="search" value="{{ request('search') }}" placeholder="Cari kode/nama..." class="w-full sm:w-96 rounded-lg border-gray-300 bg-white text-sm shadow-sm focus:border-sky-500 focus:ring-sky-500 dark:border-gray-700 dark:bg-gray-900" />
                            ...
                            @forelse ($items as $item)
                                <tr>
                                    ...
                                    <td class="py-3 text-right whitespace-nowrap">
                                        <a href="{{ route('admin.depressions.edit', $item->id) }}" class="text-sky-700 hover:text-sky-900 dark:text-sky-300 dark:hover:text-sky-200 font-semibold">Edit</a>
